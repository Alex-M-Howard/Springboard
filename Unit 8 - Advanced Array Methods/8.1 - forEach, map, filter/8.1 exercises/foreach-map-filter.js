...
function showFirstAndLast(arr) {
    arr.forEach(function (value, index) {
        return arr[index] = value[0] + value[value.length-1]
    })
    return arr;
}
...
function find(arr, searchValue) {
    ...
    let value = arr.filter(function (value) {
...
function removeVowels(str) {
    ...
    for (char of str.toLowerCase()) { arr.push(char) };
    const newString = arr.filter(function (value) {
        if (!vowels.includes(value)){return value}
    })
